...
func (service *JWTService) GenerateToken(info interface{}) (string, error) {
    ...
    timeNow := time.Now()
    expirationTime := time.Duration(config.GetInt("token_expiration"))
...
func (service *JWTService) loadSecretKey() {
    config := settings.GetInstance()
    secretKey, err := ioutil.ReadFile(config.GetString("secret_key_path"))
    if err != nil {
        panic(err)
    }
...
func (service *JWTService) loadVerifyKey() {
    config := settings.GetInstance()
    verifyKey, err := ioutil.ReadFile(config.GetString("verify_key_path"))
    if err != nil {
        panic(err)
    }
